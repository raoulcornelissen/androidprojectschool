...
public class ScannerActivity extends AppCompatActivity {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == REQUEST_IMAGE && resultCode == Activity.RESULT_OK) {
			...
			Picasso.with(ScannerActivity.this).load(destination).fit().centerCrop().into(imageView);
			resultTextView.setText("Processing");

			AsyncTask.execute(new Runnable() {
				@Override
				public void run() {
					String result = OpenALPR.Factory.create(ScannerActivity.this, ANDROID_DATA_DIR).recognizeWithCountryRegionNConfig("eu", "", destination.getAbsolutePath(), openAlprConfFile, 10);
						...
						runOnUiThread(new Runnable() {
							...
							public void run() {
								if (results == null || results.getResults() == null || results.getResults().size() == 0) {
									Toast.makeText(ScannerActivity.this, "It was not possible to detect the licence plate.", Toast.LENGTH_LONG).show();
